...
// controller-only keys: use distinct high-value markers to avoid collision
...
/// casts a weighted vote for or against the pending upgrade
fn vote_upgrade() {
...
/// executes the upgrade if a 2/3 majority is met and timelock has passed
fn execute_upgrade() {
    let mut pending = [0u8; 20];
    sget(&PENDING_IMPL_KEY, &mut pending);
    if pending == [0u8; 20] {
